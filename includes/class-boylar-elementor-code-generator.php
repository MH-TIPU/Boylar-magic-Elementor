...
class Boylar_Elementor_Code_Generator {
	...
	public function enqueue_admin_assets( $hook_suffix ) {
		// Settings page: Settings -> Boylar magic Elementor
		if ( $hook_suffix !== 'settings_page_boylar-magic-elementor' ) return;
	...
	public function register_settings() {
		...
		add_settings_section(
			...
			function () {
				echo '<p>' . esc_html__( 'Configure an OpenAI-compatible API endpoint. This can be OpenAI, a compatible gateway, or a self-hosted model server.', 'boylar-magic-elementor' ) . '</p>';
			},
			'boylar-magic-elementor'
		);
	...
	public function sanitize_settings( $input ) {
		...
		if ( ! in_array( $out['auth_header_mode'], $allowed_auth, true ) ) $out['auth_header_mode'] = $defaults['auth_header_mode'];
		...
		if ( $out['max_prompt_chars'] < 200 ) $out['max_prompt_chars'] = 200;
		if ( $out['max_prompt_chars'] > 6000 ) $out['max_prompt_chars'] = 6000;

		$out['max_image_bytes'] = isset( $input['max_image_bytes'] ) ? absint( $input['max_image_bytes'] ) : $defaults['max_image_bytes'];
		if ( $out['max_image_bytes'] < 200000 ) $out['max_image_bytes'] = 200000;
		if ( $out['max_image_bytes'] > 8000000 ) $out['max_image_bytes'] = 8000000;
		...
		if ( $out['rate_limit_per_minute'] < 1 ) $out['rate_limit_per_minute'] = 1;
		if ( $out['rate_limit_per_minute'] > 60 ) $out['rate_limit_per_minute'] = 60;

		$out['cache_ttl_seconds'] = isset( $input['cache_ttl_seconds'] ) ? absint( $input['cache_ttl_seconds'] ) : $defaults['cache_ttl_seconds'];
		if ( $out['cache_ttl_seconds'] < 0 ) $out['cache_ttl_seconds'] = 0;
		if ( $out['cache_ttl_seconds'] > 86400 ) $out['cache_ttl_seconds'] = 86400;

		$out['request_timeout'] = isset( $input['request_timeout'] ) ? absint( $input['request_timeout'] ) : $defaults['request_timeout'];
		if ( $out['request_timeout'] < 10 ) $out['request_timeout'] = 10;
		if ( $out['request_timeout'] > 120 ) $out['request_timeout'] = 120;

		$out['max_output_tokens'] = isset( $input['max_output_tokens'] ) ? absint( $input['max_output_tokens'] ) : $defaults['max_output_tokens'];
		if ( $out['max_output_tokens'] < 200 ) $out['max_output_tokens'] = 200;
		if ( $out['max_output_tokens'] > 4000 ) $out['max_output_tokens'] = 4000;
	...
	public function field_api_base_url() {
		$s = $this->get_settings();
		printf(
			'<input type="url" class="regular-text" name="%s[api_base_url]" value="%s" />',
			esc_attr( self::OPT_KEY ),
			esc_attr( $s['api_base_url'] )
		);
		echo '<p class="description">' . esc_html__( 'Must be OpenAI-compatible (supports /chat/completions).', 'boylar-magic-elementor' ) . '</p>';
	}
	...
	public function field_api_key() {
		...
		if ( $using_constant ) {
			...
			echo '<p class="description">' . esc_html__( 'Stored in WordPress options. For best security, define BOYLAR_OPENAI_API_KEY in wp-config.php instead.', 'boylar-magic-elementor' ) . '</p>';
		}
	}
...
PROMPT;
	...
	private function sanitize_box( array $box ) {
		...
		if ( ! in_array( $unit, [ 'px', '%', 'em', 'rem', 'vh', 'vw' ], true ) ) $unit = 'px';
